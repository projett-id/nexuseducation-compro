...
@section('content')
<form class="card" action="{{ route('program-types.update', $programType->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="card-body">
        ...
        <div class="mb-3">
            ...
            <select name="country_id" class="form-control">
                ...
                <option value="{{ $c->id }}" {{ $programType->country_id == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group mb-3">
            <label>Section</label>
            <input type="text" name="title" class="form-control" required autocomplete="off" value="{{ $programType->title }}">
            @error('title') <small class="text-danger">{{ $message }}</small> @enderror
        </div>
        <div class="mb-3">
            <label>Content</label>
            <textarea name="content" class="form-control tinymce" rows="5">{{ $programType->content }}</textarea>
